...
using AircraftApp.Authentication;
using AircraftApp.Entities;
using AircraftApp.EntityFrameworkCore;

namespace AircraftApp.Web.Host.Controllers
{
        ...
        public ActionResult Authenticate([FromBody] AuthenticateModel model)
        {
            try
            {
                ...
                if (user.PasswordHash != null && user.PasswordHash.StartsWith("PBKDF2$"))
                    pwOk = AircraftApp.Users.PasswordHelper.VerifyPbkdf2(model.Password, user.PasswordHash);
                else
