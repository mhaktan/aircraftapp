...
interface MaintenanceLogCreateProps {
  open: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export const MaintenanceLogCreate: React.FC<MaintenanceLogCreateProps> = ({ open, onClose, onSuccess }) => {
  const [form, setForm] = useState<Partial<MaintenanceLogRecord>>({});
  const setField = (name: string, value: unknown) => setForm((p) => ({ ...p, [name]: value }));
  const { triggerFlows } = useFlows();

  const mutation = useMutation({
    mutationFn: (values: Partial<MaintenanceLogRecord>) => dataProvider.create('MaintenanceLog', values),
    onSuccess: (_data, values) => { triggerFlows('create', 'MaintenanceLog', values as Record<string, unknown>); onSuccess(); onClose(); setForm({}); },
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: '20px 28px 0', flexShrink: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>Create MaintenanceLog</h2>
