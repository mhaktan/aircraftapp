...
interface SparePartEditProps {
  record: SparePartRecord | null;
  onClose: () => void;
  onSuccess: () => void;
}

export const SparePartEdit: React.FC<SparePartEditProps> = ({ record, onClose, onSuccess }) => {
  const [form, setForm] = useState<Partial<SparePartRecord>>(record ?? {});
  ...
  const mutation = useMutation({
    mutationFn: (values: Partial<SparePartRecord>) =>
      dataProvider.update('SparePart', record!.id, values),
    onSuccess: (_data, values) => { triggerFlows('update', 'SparePart', values as Record<string, unknown>); onSuccess(); onClose(); },
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: '20px 28px 0', flexShrink: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>Edit SparePart</h2>
