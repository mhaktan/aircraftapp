...
interface MaintenanceTypeEditProps {
  record: MaintenanceTypeRecord | null;
  onClose: () => void;
  onSuccess: () => void;
}

export const MaintenanceTypeEdit: React.FC<MaintenanceTypeEditProps> = ({ record, onClose, onSuccess }) => {
  const [form, setForm] = useState<Partial<MaintenanceTypeRecord>>(record ?? {});
  ...
  const mutation = useMutation({
    mutationFn: (values: Partial<MaintenanceTypeRecord>) =>
      dataProvider.update('MaintenanceType', record!.id, values),
    onSuccess: (_data, values) => { triggerFlows('update', 'MaintenanceType', values as Record<string, unknown>); onSuccess(); onClose(); },
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: '20px 28px 0', flexShrink: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>Edit MaintenanceType</h2>
