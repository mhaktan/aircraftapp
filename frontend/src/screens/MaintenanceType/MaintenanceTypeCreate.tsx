...
interface MaintenanceTypeCreateProps {
  open: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export const MaintenanceTypeCreate: React.FC<MaintenanceTypeCreateProps> = ({ open, onClose, onSuccess }) => {
  const [form, setForm] = useState<Partial<MaintenanceTypeRecord>>({});
  const setField = (name: string, value: unknown) => setForm((p) => ({ ...p, [name]: value }));
  const { triggerFlows } = useFlows();

  const mutation = useMutation({
    mutationFn: (values: Partial<MaintenanceTypeRecord>) => dataProvider.create('MaintenanceType', values),
    onSuccess: (_data, values) => { triggerFlows('create', 'MaintenanceType', values as Record<string, unknown>); onSuccess(); onClose(); setForm({}); },
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: '20px 28px 0', flexShrink: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>Create MaintenanceType</h2>
