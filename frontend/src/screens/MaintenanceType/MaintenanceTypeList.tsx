...
import { MaintenanceTypeCreate } from './MaintenanceTypeCreate';
import { MaintenanceTypeEdit } from './MaintenanceTypeEdit';
...
export const MaintenanceTypeList: React.FC = () => {
  const list = useListQuery<MaintenanceTypeRecord>({ resource: 'MaintenanceType' });
  const [showCreate, setShowCreate] = useState(false);
  const [editRecord, setEditRecord] = useState<MaintenanceTypeRecord | null>(null);
  const [selectedRows, setSelectedRows] = useState<MaintenanceTypeRecord[]>([]);
  const { triggerFlows } = useFlows();
  const del = useDeleteMutation('MaintenanceType', (ids) => { ids.forEach(id => triggerFlows('delete', 'MaintenanceType', { id })); });
  ...
  return (
    <>
      <ListPageLayout
        ...
        onSelectionChange={(rows) => setSelectedRows(rows as MaintenanceTypeRecord[])}
        ...
        headerActions={<>
          ...
          <TkButton label="+ Create MaintenanceType" variant="primary" onTkClick={() => setShowCreate(true)} />
        </>}
      />
      ...
      />
      <MaintenanceTypeCreate open={showCreate} onClose={() => setShowCreate(false)} onSuccess={list.invalidate} />
      <MaintenanceTypeEdit record={editRecord} onClose={() => setEditRecord(null)} onSuccess={list.invalidate} />
