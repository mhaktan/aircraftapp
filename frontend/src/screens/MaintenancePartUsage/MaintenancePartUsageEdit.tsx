...
interface MaintenancePartUsageEditProps {
  record: MaintenancePartUsageRecord | null;
  onClose: () => void;
  onSuccess: () => void;
}

export const MaintenancePartUsageEdit: React.FC<MaintenancePartUsageEditProps> = ({ record, onClose, onSuccess }) => {
  const [form, setForm] = useState<Partial<MaintenancePartUsageRecord>>(record ?? {});
  ...
  const mutation = useMutation({
    mutationFn: (values: Partial<MaintenancePartUsageRecord>) =>
      dataProvider.update('MaintenancePartUsage', record!.id, values),
    onSuccess: (_data, values) => { triggerFlows('update', 'MaintenancePartUsage', values as Record<string, unknown>); onSuccess(); onClose(); },
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: '20px 28px 0', flexShrink: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>Edit MaintenancePartUsage</h2>
        ...
        <form onSubmit={(e) => { e.preventDefault(); mutation.mutate(form); }} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          <div style={{ flex: 1, overflowY: 'auto', padding: '20px 28px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                ...
                <div>
                  <LookupSelect label="Bakım Kaydı *" resource="MaintenanceLog" value={String(form.maintenanceLogId ?? '')} onChange={(v) => setField('maintenanceLogId', v)} displayField="performedBy" />
                </div>
                <div>
