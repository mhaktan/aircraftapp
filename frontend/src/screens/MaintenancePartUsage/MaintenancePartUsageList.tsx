...
import { MaintenancePartUsageCreate } from './MaintenancePartUsageCreate';
import { MaintenancePartUsageEdit } from './MaintenancePartUsageEdit';
...
type MaintenancePartUsageRecord = {
  id: string | number;
  quantityUsed: number;
  notes?: string;
...
const COLUMNS: TableColumn[] = [
  ...
  { field: 'maintenanceLogId', header: 'Bakım Kaydı', sortable: true },
  { field: 'sparePartId', header: 'Yedek Parça', sortable: true },
];

// ---------------------------------------------------------------------------
// MaintenancePartUsageList
// ---------------------------------------------------------------------------

export const MaintenancePartUsageList: React.FC = () => {
  const list = useListQuery<MaintenancePartUsageRecord>({ resource: 'MaintenancePartUsage' });
  const [showCreate, setShowCreate] = useState(false);
  const [editRecord, setEditRecord] = useState<MaintenancePartUsageRecord | null>(null);
  const [selectedRows, setSelectedRows] = useState<MaintenancePartUsageRecord[]>([]);
  const { triggerFlows } = useFlows();
  const del = useDeleteMutation('MaintenancePartUsage', (ids) => { ids.forEach(id => triggerFlows('delete', 'MaintenancePartUsage', { id })); });
  ...
  return (
    <>
      <ListPageLayout
        ...
        onSelectionChange={(rows) => setSelectedRows(rows as MaintenancePartUsageRecord[])}
        ...
        headerActions={<>
          ...
          <TkButton label="+ Create MaintenancePartUsage" variant="primary" onTkClick={() => setShowCreate(true)} />
        </>}
      />
      ...
      />
      <MaintenancePartUsageCreate open={showCreate} onClose={() => setShowCreate(false)} onSuccess={list.invalidate} />
      <MaintenancePartUsageEdit record={editRecord} onClose={() => setEditRecord(null)} onSuccess={list.invalidate} />
